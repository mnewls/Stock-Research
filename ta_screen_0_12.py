...
def TA_screening(stock):
    ...
    def add_labels(df):
        window_size = 15
                        ...
                        #30 intervals = 1 hr <<OLD
                #using 1 day intervals >> window size = 5 = 1 trading week
                
        #print(len(df.index))
        list_labels_len = [None] * (window_size-1)

        list_labels = pd.DataFrame(list_labels_len)
        ...
        for row_counter in range(total_rows):
            if row_counter >= window_size - 1:
                ...
                if max_index == window_middle:
                    list_labels.at[window_middle] = 0
                    #list_labels.append(0)
                elif min_index == window_middle:
                    list_labels.at[window_middle] = 1
                    #list_labels.append(1)
                else:
                    list_labels.at[window_middle] = 2
    ...
    plt.figure()
    plt.plot(test['labels'], test_pred)
    plt.figure()
    plot_confusion_matrix(model, X_test_scaled, y_test_scaled, display_labels=['Sell','Buy','Hold'], normalize='true')
    plt.show()
    print(test_pred[-1])

    buy_list = test.index[test['labels']==1].tolist()
    sell_list = test.index[test['labels']==0].tolist()

    plt.plot(test_scaled.index, test_close_vals, 'g+', markevery=buy_list)
    plt.plot(test_scaled.index, test_close_vals, 'rv', markevery=sell_list)
    plt.show()

    #last class label
    #plt.ylabel('BUY => 1, SELL => 0, HOLD => 2')

# there is more to be done here > why does it break when setting the trade window length?
